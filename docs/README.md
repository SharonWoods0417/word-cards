...
#### 4.2 PDF 导出（强化版）
- [ ] 集成 `pdf-lib` 库（替代 jsPDF）
- [ ] 使用物理尺寸单位（mm/pt）而非 px
- [ ] 设置 A4 纸张尺寸（210mm × 297mm）
- [ ] 实现固定 3 行 × 2 列布局
- [ ] 设置卡片尺寸为 90mm × 60mm
- [ ] 使用固定坐标精确定位每张卡片
- [ ] 生成正面页面（图片+单词+音标+拼读）
...
#### 4.3 卡片内容结构
- [ ] 正面布局：图片（上40%）+ 单词+音标（靠近）+ 拼读拆分
- [ ] 反面布局：灰底中文释义（上40%）+ 英文例句+中文翻译（紧贴）
- [ ] 确保打印时排版与网页预览效果完全一致
...
#### 5.1 API 集成准备
- [ ] 配置 OpenRouter API 密钥管理
- [ ] 配置 Pexels API 密钥管理
- [ ] 创建 API 调用工具函数
- [ ] 添加环境变量配置

#### 5.2 文本字段补全（OpenRouter API）
- [ ] 实现 OpenRouter API 调用函数
- [ ] 创建文本补全提示词模板
- [ ] 补全 phonetic（音标）字段
- [ ] 补全 chinese（中文释义）字段
- [ ] 补全 example（英文例句）字段
- [ ] 补全 translation（例句翻译）字段

#### 5.3 图片字段补全（Pexels API）
- [ ] 实现 Pexels API 调用函数
- [ ] 根据单词搜索相关图片
- [ ] 补全 imageUrl 字段（搜索首张相关图片）
- [ ] 添加图片质量筛选
