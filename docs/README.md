...
### 功能特点
- **专业规则**：基于 phonics_split_rules_v4.0_syllable.md 专业规则实现
- **自动生成**：输入单词时自动生成音节拆分
- **教学标准**：符合英语教学音节拆分标准
- **智能识别**：支持复合词、前后缀、元音组合等复杂情况

### 拆分规则
1. **复合词优先**：football → foot-ball
2. **Final Stable Syllable**：apple → ap-ple
3. **前后缀处理**：disappear → dis-a-ppear
4. **VC/CV 拆分**：rabbit → rab-bit
5. **元音组合不拆**：paint → paint
6. **Magic-e 不拆**：cake → cake
7. **R控制元音**：market → mar-ket

### 使用方式
- **手动输入**：输入单词时自动生成拆分
- **CSV导入**：导入时自动补充缺失的拆分
- **AI补全**：补全时自动生成拆分
- **测试工具**：提供专门的测试页面验证拆分效果
